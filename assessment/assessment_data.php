  ...
  <main class="app-content">
    ...
    <div class="row">
      ...
      $sql = mysqli_query($conn,"SELECT *,b.ws_id,f.m_name as m_sender,g.m_name as m_name,b.place_id as pl,b.dv_id,f.m_id as sender_id,g.m_id as m_id
       ...
       on b.ws_id = i.ws_id
       where b.ws_job_end !='' and b.ws_id = '".$_GET['id']."'
      ...
      while($rs = mysqli_fetch_assoc($sql)){ 
        ...
        if(strtotime($rs['ws_assess_date']) >= strtotime(date('Y-m-d')) or $rs['ws_assess_date'] ==''){ ?>  
          <div class="col-md-12 p-1">
            <div class="tile"> 
              <div class="col-md-12">
                <h6 class="text-right"><a href="assessment.php" title=""> ย้อนกลับ <i class="fa fa-reply-all"></i></a></h6>
              </div>
              <hr>
              <!--//////////-->
      ...
      <div class="modal-footer">
       <button type="text" class="btn btn-dark " id="update">ยืนยันการแก้ไข</button>
      ...
      <div class="modal-footer"><!-----////// footer------>
        <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
          ...
          <div class="btn-group" role="group">
            <button class="btn btn-primary dropdown-toggle " id="btnGroupDrop1" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">เลือก</button>
            <div class="dropdown-menu dropdown-menu-right">
              <a class="dropdown-item insert_success" 
              data-id = "<?=$rs['ws_id']?>"
              >ผ่าน</a>
              <a class="dropdown-item faild"
              data-id = "<?=$rs['ws_id']?>"
              data-toggle="modal" 
              data-target="#insert_ws_assess"
              >ไม่ผ่าน</a>
            </div>
          </div>
        </div>
