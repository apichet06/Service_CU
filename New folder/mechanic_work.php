...
<body class="app sidebar-mini">
  ...
  <main class="app-content">
          ...
          <div class="col-md-6">
            ...
            <div class="form-group col-md-4">
              <select name="action" class="form-control">
                <option value="">--- ลักษณะ ---</option>
                <option value="ปิดงาน">ปิดงานแล้ว</option>
                <option value="sla">ตก SLA</option>
                <option value="งานคงค้าง">งานคงค้าง</option>
                <option value="คืนงาน">คืนงาน</option>
              </select>
            </div>
    ...
    $sql01 = mysqli_query($conn,"SELECT z_name FROM zone WHERE z_id = '".$z_id[0]."' ");
    $rs01  = mysqli_fetch_assoc($sql01);

    $sql02 = mysqli_query($conn,"SELECT c_name FROM category WHERE c_id = '".$_POST['c_id']."' ");
    $rs02  = mysqli_fetch_assoc($sql02);

    $sql03 = mysqli_query($conn,"SELECT m_name FROM member WHERE m_id = '".$_POST['m_id']."' ");
    $rs03  = mysqli_fetch_assoc($sql03);          
    ?>
    <div class="row justify-content-center">
      <div class="col-md-12">
        <?php   if($_POST['action']=="ปิดงาน"){
          echo "<hr><strong class='text-secondary text-center'>รายการที่สืบค้น : (รายการที่ปิดงาน) (ตั้งแต่วันที่ ".$_POST['date_1']." ถึง ".$_POST['date_2'].")</strong>";
          echo $rs01['z_name'] =='' ? '' : "<strong class='text-secondary'> (โซนพื้นที่ ". $rs01['z_name'].")</strong>"; 
          echo $rs02['c_name'] =='' ? '' : "<strong class='text-secondary'> (ประเภท ". $rs02['c_name'].")</strong>";
          echo $rs03['m_name'] =='' ? '' : "<strong class='text-secondary'>  (ชื่อ ". $rs03['m_name'].")</strong>";
          
          
        }else if($_POST['action']=="คืนงาน"){
          echo "<hr><strong class='text-secondary text-center'>รายการที่สืบค้น : (รายการที่คืนงาน) (ตั้งแต่วันที่ ".$_POST['date_1']." ถึง ".$_POST['date_2'].")</strong>";
          echo $rs01['z_name'] =='' ? '' : "<strong class='text-secondary'> (โซนพื้นที่ ". $rs01['z_name'].")</strong>"; 
          echo $rs02['c_name'] =='' ? '' : "<strong class='text-secondary'> (ประเภท ". $rs02['c_name'].")</strong>";
          echo $rs03['m_name'] =='' ? '' : "<strong class='text-secondary'>  (ชื่อ ". $rs03['m_name'].")</strong>";
        }else if($_POST['action']=="sla"){
          echo "<hr><strong class='text-secondary text-center'>รายการที่สืบค้น : (รายการที่ตก SLA แล้ว) (ตั้งแต่วันที่ ".$_POST['date_1']." ถึง ".$_POST['date_2'].")</strong>";
          echo $rs01['z_name'] =='' ? '' : "<strong class='text-secondary'> (โซนพื้นที่ ". $rs01['z_name'].")</strong>"; 
          echo $rs02['c_name'] =='' ? '' : "<strong class='text-secondary'> (ประเภท ". $rs02['c_name'].")</strong>";
          echo $rs03['m_name'] =='' ? '' : "<strong class='text-secondary'> (ชื่อ ". $rs03['m_name'].")</strong>";
        }else if($_POST['action']=="งานคงค้าง"){
          echo "<hr><strong class='text-secondary text-center'>รายการที่สืบค้น : (รายการที่ งานคงค้าง) (ตั้งแต่วันที่ ".$_POST['date_1']." ถึง ".$_POST['date_2'].")</strong>";
          echo $rs01['z_name'] =='' ? '' : "<strong class='text-secondary'> (โซนพื้นที่ ". $rs01['z_name'].")</strong>"; 
          echo $rs02['c_name'] =='' ? '' : "<strong class='text-secondary'> (ประเภท ". $rs02['c_name'].")</strong>";
          echo $rs03['m_name'] =='' ? '' : "<strong class='text-secondary'> (ชื่อ ". $rs03['m_name'].")</strong>";
        } ?>
        ...
        if($_POST['date_1']){
          ...
          if($_POST['action']=='ปิดงาน'){
           ...
           $sql = mysqli_query($conn,"SELECT *,b.m_name as m_name,f.m_name as sender,g.ws_id as g_ws_id,a.ws_id as ws_id FROM worksheet a 
            ...
            on a.c_id = h.c_id
            Where date(a.ws_date) >= '".$_POST['date_1']."' and date(a.ws_date) <= '".$_POST['date_2']."' and a.z_id Like '%".$z_id[0]."' and a.c_id Like '%".$_POST['c_id']."%' and a.m_id Like '%".$_POST['m_id']."%' and 
            a.panding_note ='' and  a.ws_job_end !=''
            ORDER BY g.t_date DESC");
          ...
          $sql = mysqli_query($conn,"SELECT *,b.m_name as m_name,f.m_name as sender,g.ws_id as g_ws_id,a.ws_id as ws_id FROM worksheet a 
            ...
            LEFT JOIN category h 
            on a.c_id = h.c_id
            Where date(g.t_date) >= '".$_POST['date_1']."' and date(g.t_date) <= '".$_POST['date_2']."' and a.z_id Like '%".$z_id[0]."' and a.c_id Like '%".$_POST['c_id']."%' and g.m_id Like '%".$_POST['m_id']."%'
            ORDER BY g.t_date DESC");

        }elseif($_POST['action']=='sla'){

          $date = date("Y-m-d H:i:s");
          $sql = mysqli_query($conn,"SELECT *,b.m_name as m_name,f.m_name as sender,a.ws_id as ws_id FROM worksheet a 
            INNER JOIN member b 
            on a.m_id = b.m_id
            LEFT JOIN place c 
            on a.place_id = c.place_id
            ...
            LEFT JOIN category  h 
            on a.c_id = h.c_id
            WHERE (date(a.ws_date) >= '".$_POST['date_1']."' and date(a.ws_date) <= '".$_POST['date_2']."' and a.z_id Like '%".$z_id[0]."') and a.c_id Like '%".$_POST['c_id']."%' and a.m_id Like '%".$_POST['m_id']."%' and a.ws_end_date <= '$date' and ISNULL(a.ws_job_end) and a.ws_end_date !='0000-00-00 00:00:00' 
            ORDER BY a.ws_id DESC ");
        }elseif($_POST['action']=='งานคงค้าง'){

          $sql = mysqli_query($conn,"SELECT *,f.m_name as sender,g.m_name as m_name FROM  worksheet a
            INNER JOIN zone b 
            on a.z_id = b.z_id
            LEFT JOIN place c 
            on a.place_id = c.place_id
            ...
            INNER JOIN member g 
            on a.m_id = g.m_id 
            Where a.panding_note !='' and ISNULL(a.ws_job_end) and (date(a.ws_date) >= '".$_POST['date_1']."' and date(a.ws_date) <= '".$_POST['date_2']."' and a.z_id Like '%".$z_id[0]."') and a.c_id Like '%".$_POST['c_id']."%' and a.m_id Like '%".$_POST['m_id']."%'
            ORDER BY a.ws_id DESC");
  ...
  //       echo "ผ่าน SLA";
  //     }
  //   }else if($rs['ws_end_date'] != '0000-00-00 00:00:00' and $rs['ws_end_date'] != ''){
  //    echo $date;
  //  }else{
  //   echo  "ไม่มีกำหนดปิดงาน";
  // } 
